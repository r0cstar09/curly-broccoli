...
**Never commit secrets to git.** All sensitive values are stored in environment variables.

1. Copy the template: `cp .env.example .env`
2. Fill in your values in `.env` (this file is gitignored)
3. For production: use your platform's secrets manager (Netlify Environment Variables, Vercel, AWS Secrets Manager, etc.)

The app runs without secrets for browsing; checkout requires Stripe keys to be configured.

## Services

This repository connects to related services to power payments, emails, and map embeds. Visit the [`astro.config.ts`](astro.config.ts) file for an overview of all environment variables and access permissions required for each.
